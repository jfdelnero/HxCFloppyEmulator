	...
	for(j=0;j<floppydisk->floppyNumberOfTrack;j++)
	{
		...
		for(i=0;i<floppydisk->floppyNumberOfSide;i++)
		{
			...
			if(tracktype)
			{
				...
				floppydisk->tracks[j]->sides[i]=tg_generateTrack(trackdata,sectorsize,floppydisk->floppySectorPerTrack,(unsigned char)j,(unsigned char)i,(unsigned char)firstsectorid,interleave,(unsigned char)(curskew),floppydisk->floppyBitRate,rpm,tracktype,gap3len,0,2500|NO_SECTOR_UNDER_INDEX,-2500);
			}
		}
	}
