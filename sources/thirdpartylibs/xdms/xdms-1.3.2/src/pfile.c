...
static USHORT Process_Track(FILE *, HXCFILE *, UCHAR *, UCHAR *, USHORT, USHORT, USHORT);
...
USHORT Process_File(char *iname, HXCFILE *fo, USHORT cmd, USHORT opt, USHORT PCRC, USHORT pwd){
	FILE *fi;
	...
	if (cmd != CMD_VIEW) {
		...
		{
			ret = Process_Track(fi,(HXCFILE*)NULL,b1,b2,cmd,opt,(USHORT)((geninfo & 2)?pwd:0));
		}
		else {
			while ( (ret=Process_Track(fi,fo,b1,b2,cmd,opt,(USHORT)((geninfo & 2)?pwd:0))) == NO_PROBLEM ) ;
...
static USHORT Process_Track(FILE *fi, HXCFILE *fo, UCHAR *b1, UCHAR *b2,
			    USHORT cmd, USHORT opt, USHORT pwd)
{
	...
	if ((cmd == CMD_UNPACK) && (number<80) && (unpklen>2048)) {
		...
		if (HXC_fwrite(b2, 1, (size_t) unpklen, fo) != unpklen)
	...
	switch (cmode){
		...
		case 6:
			...
			if (cmode==5) {
				...
				/*   Heavy 2   */
				if (Unpack_HEAVY(b1,b2,(UCHAR)(flags | (UCHAR)8),pklen2)) return ERR_BADDECR;
			}
			if (flags & 4) {
