...
// Contains: Acorn ADF floppy image loader and plugins interfaces
//
...
int ACORNADF_libIsValidDiskFile(HXCFLOPPYEMULATOR* floppycontext,char * imgfile)
{
	...
	char * filepath;
	FILE *f;
	floppycontext->hxc_printf(MSG_DEBUG,"ACORNADF_libIsValidDiskFile %s",imgfile);
	if(imgfile)
	{
	...
	{		
		...
		for(j=0;j<floppydisk->floppyNumberOfTrack;j++)
		{
			...
			for(i=0;i<floppydisk->floppyNumberOfSide;i++)
			{
				...
				currentcylinder->sides[i]=tg_generatetrack(trackdata,sectorsize,floppydisk->floppySectorPerTrack,(unsigned char)j,(unsigned char)i,0,interleave,(unsigned char)(0),floppydisk->floppyBitRate,currentcylinder->floppyRPM,trackformat,gap3len,2500);
				fillindex(currentcylinder->sides[i]->tracklen-1,currentcylinder->sides[i],2500,TRUE,1);
			}
		}
