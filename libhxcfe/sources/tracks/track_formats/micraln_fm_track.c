		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if( tmp_buffer[0] == LUT_ByteBitsInverter[0xFD] )
				{
					...
					if( checksum == LUT_ByteBitsInverter[tmp_buffer[ 4 ]] )
					{ // checksum ok !!!
 						floppycontext->hxc_printf(MSG_DEBUG,"Valid FM Micral N sector found - Cyl:%d Side:%d Sect:%d Size:%d",LUT_ByteBitsInverter[ tmp_buffer[2] ],tmp_buffer[5],LUT_ByteBitsInverter[ tmp_buffer[3] ],sectorsize[tmp_buffer[7]&0x7]);
					}
					else
					{
						floppycontext->hxc_printf(MSG_DEBUG,"Bad FM Micral N header found - Cyl:%d Side:%d Sect:%d Size:%d",LUT_ByteBitsInverter[ tmp_buffer[2] ],tmp_buffer[5],LUT_ByteBitsInverter[ tmp_buffer[3] ],sectorsize[tmp_buffer[7]&0x7]);
...
void tg_addMicralNSectorToTrack(track_generator *tg,HXCFE_SECTCFG * sectorconfig,HXCFE_SIDE * currentside)
{
	...
	uint8_t  checksum,c,for_carry,carry;
	...
	for(i=0;i<17;i++)
	{
		pushTrackCode(tg,0x00,0xFF,currentside,MICRALN_HS_SD);
	}
	...
	// sync
	pushTrackCode(tg,0xFF,0xFF,currentside,MICRALN_HS_SD);
	...
	for(i=0;i<128;i++)
	{
		...
		for_carry = ((c ^ checksum) ^ 0xff) & ((c + checksum) ^ c);
		carry = (for_carry & 0x80) ? 1 : 0;
		checksum = checksum + c + carry;
	}
