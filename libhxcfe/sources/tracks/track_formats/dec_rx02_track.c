...
// Contains: DEC RX02 track support
//
...
//;            1   1    1   1    1   1       0   1   -- FD
						...
						if(bit_offset != -1)
						{
							...
							#define SECT_DATA_SIZE (1+sector->sectorsize+2)
							CRC16_Init(&CRC16_High,&CRC16_Low,(unsigned char*)crctable,0x1021,0xFFFF);
...
int write_dec_rx02_sectordata(HXCFE* floppycontext,HXCFE_SIDE * track,HXCFE_SECTCFG * sector,unsigned char * buffer,int buffersize)
{
	int bit_offset,i;
	unsigned char CRC16_High;
	unsigned char CRC16_Low;
