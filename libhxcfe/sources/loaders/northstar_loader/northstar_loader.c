...
int Northstar_libLoad_DiskFile(HXCFE_IMGLDR * imgldr_ctx,HXCFE_FLOPPY * floppydisk,char * imgfile,void * parameters)
{
	...
	imgldr_ctx->hxcfe->hxc_printf(MSG_DEBUG,"Northstar_libLoad_DiskFile %s",imgfile);
	...
	if( !(filesize % (35*10*512) ) )
	{
		...
		floppydisk->floppyNumberOfSide = filesize / ( floppydisk->floppySectorPerTrack * 512 * floppydisk->floppyNumberOfTrack);
		...
		if(trackdata)
		{
			...
			for(i=0;i<floppydisk->floppyNumberOfSide;i++)
			{
				for(j=0;j<floppydisk->floppyNumberOfTrack;j++)
				{
					...
					hxc_fread(trackdata,(floppydisk->floppySectorPerTrack*512),f);
