	...
	if((filesize!=0) && (header_buffer[0]==0x0D) && (header_buffer[1]==0x0A))
	{
		...
		for(j=0;j<floppydisk->floppyNumberOfTrack;j++)
		{
			...
			for(i=0;i<floppydisk->floppyNumberOfSide;i++)
			{
				...
				currentcylinder->sides[i]=tg_generateTrackEx(floppydisk->floppySectorPerTrack,sectorconfig,interleave,(unsigned char)(((j<<1)|(i&1))*skew),floppydisk->floppyBitRate,rpm,trackformat,0,2500,-2500);
