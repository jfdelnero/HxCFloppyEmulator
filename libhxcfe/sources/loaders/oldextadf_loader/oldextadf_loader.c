...
int OLDEXTADF_libIsValidDiskFile( HXCFE_IMGLDR * imgldr_ctx, HXCFE_IMGLDR_FILEINFOS * imgfile )
{
	imgldr_ctx->hxcfe->hxc_printf(MSG_DEBUG,"OLDEXTADF_libIsValidDiskFile");

	if( hxc_checkfileext(imgfile->path,"adf",SYS_PATH_TYPE) )
	{
		if(!strncmp((char*)imgfile->file_header,"UAE--ADF",8))
		{
			...
			if(trackindex<numberoftrack)
			{
				...
				if(tracksize)
				{
					...
					else
					{
						...
						currentcylinder->sides[i]=tg_generateTrack(trackdata,sectorsize,(unsigned short)(tracksize/sectorsize),(unsigned char)j,(unsigned char)i,0,interleave,((j<<1)|(i&1))*skew,floppydisk->floppyBitRate,currentcylinder->floppyRPM,trackformat,gap3len,0,2500,-11150);
