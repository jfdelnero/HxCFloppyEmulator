...
RETCODE adfMountHd(struct Device *dev)
{
    ...
    while( next!=-1 ) {
        ...
        vol->volName = (char*)calloc(1,len+1);
        if (!vol->volName) { 
            adfFreeTmpVolList(listRoot);
    ...
    dev->volList = (struct Volume**)calloc(1,sizeof(struct Volume*) * dev->nVol);
    if (!dev->volList) { 
        adfFreeTmpVolList(listRoot);
...
RETCODE adfCreateHdHeader(struct Device* dev, int n, struct Partition** partList )
{
    ...
    for(i=0; i<dev->nVol; i++) {
        ...
        strncpy(part.dosType, "DOS", sizeof(part.dosType));
        part.dosType[sizeof(part.dosType) - 1] = 0;
    ...
    strncpy(fshd.dosType, "DOS", sizeof(fshd.dosType));
    fshd.dosType[sizeof(fshd.dosType) - 1] = 0;
 ...
 * fills dev->volList[]
 */
RETCODE adfCreateFlop(struct Device* dev, char* volName, int volType,struct DateTime * voldate )
{
    if (dev==NULL) {
    ...
    dev->volList[0] = adfCreateVol( dev, 0L, 80L, volName, volType,voldate );
...
/*struct Volume *vol;*/
    ...
    dev->volList =(struct Volume**) calloc(1,sizeof(struct Volume*)*n);
    if (!dev->volList) {
...
adfWritePARTblock(struct Device *dev, int32_t nSect, struct bPARTblock* part)
{
    ...
    newSum = adfNormalSum(buf, 8, LOGICAL_BLOCK_SIZE);
    swLong(buf+8, newSum);
/*    *(int32_t*)(buf+8) = swapLong((unsigned char*)&newSum);*/
