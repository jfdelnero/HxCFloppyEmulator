...
// Contains: BBC floppy image loader and plugins interfaces
//
...
int DSD_libIsValidDiskFile(HXCFLOPPYEMULATOR* floppycontext,char * imgfile)
{
	...
	char * filepath;
	FILE * f;
	floppycontext->hxc_printf(MSG_DEBUG,"DSD_libIsValidDiskFile %s",imgfile);
	if(imgfile)
	{
		...
		if(pathlen!=0)
		{
			...
			if(filepath!=0)
			{
				...
				if((strstr( filepath,".dsd" )!=NULL) || (strstr( filepath,".ssd" )!=NULL))
				{
